...
class PostController extends Controller
{
    ...
    public function BlogCatStore(Request $request)
    {
    	$validateDate = $request->validate([
    		...
    		'category_name_th' => 'required|max:255',

    	]);

    	$data = array();
    	$data['category_name_en'] = $request->category_name_en;
    	$data['category_name_th'] = $request->category_name_th;
    ...
    {
    	$blogcateEdit = DB::table('post_category')->where('id',$id)->first();

    	return view('admin.blog.category.edit',compact('blogcateEdit'));
     ...
     public function store(Request $request)
     {
     	...
     	$data['post_title_th'] = $request->post_title_th;
     	$data['category_id'] = $request->category_id;
     	$data['details_en'] = $request->details_en;
     	$data['details_th'] = $request->details_th;
     	
     	$post_image = $request->file('post_image');

     	if ($post_image) {
    		$post_image_name = hexdec(uniqid()).'.'.$post_image->getClientOriginalExtension();
    		Image::make($post_image)->resize(400,250)->save('public/media/posts/'.$post_image_name);
        	...
        	return Redirect()->route('list.blogpost')->with($notification);
    	}else{
